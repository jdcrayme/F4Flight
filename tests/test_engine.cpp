...
TEST_F(EngineTest, AfterburnerBoost) {
    ...
    EXPECT_GT(s_ab.thrust, s_mil.thrust * 1.4); // AB much higher than MIL
    // AB at sea level static: thrust = 23,770 lbf
    const double expected_ab_accel = 23770.0 * GRAVITY / 24000.0;
    EXPECT_NEAR(s_ab.thrust, expected_ab_accel, 5.0);
}
